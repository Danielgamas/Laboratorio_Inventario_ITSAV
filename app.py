...
def registro():
    if request.method == "POST":
        ...
        if usuario_existente or cuenta_existente:
        ...
        try:
            ...
            db.session.rollback()
            flash("No se pudo registrar. Verifica que la matrícula o correo no estén repetidos.", "danger")
            return redirect(url_for("registro"))
...
def prestamos():
    ...
    prestamos = consulta.order_by(Prestamo.fecha_solicitud.desc()).all()
...
@app.route("/portal/equipos/<int:id>/solicitar", methods=["POST"])
@estudiante_required
...
def mis_solicitudes():
    ...
    return render_template("mis_solicitudes.html", prestamos=prestamos)
